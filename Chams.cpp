...
IMaterial* CreateMaterial(bool Ignore, bool Lit, bool Wireframe)
{
	...
	IMaterial* createdMaterial = Interfaces::g_MaterialSystem->CreateMaterial(name, keyValues);
...
IMaterial* CreateMaterial2(std::string type, std::string texture, bool ignorez, bool nofog, bool model, bool nocull, bool halflambert)
{
	...
	return Interfaces::g_MaterialSystem->CreateMaterial(materialName.c_str(), keyValues);
}
...
void ForceMaterial(Color color, IMaterial* material, bool useColor, bool forceMaterial)
{
	if (useColor)
	{
		...
		Interfaces::g_RenderView->SetBlend(1.0f);
		Interfaces::g_RenderView->SetColorModulation(temp);
	}

	if (forceMaterial)
		Interfaces::g_ModelRender->ForcedMaterialOverride(material);
	else
