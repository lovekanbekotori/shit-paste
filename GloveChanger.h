...
{
	auto clazz = Interfaces::g_CHLClient->GetAllClasses();
...
{
	ClientClass* pClient = Interfaces::g_CHLClient->GetAllClasses();
	if (!pClient)
		return false;
...
void GloveChanger()
{
	auto local_id = Interfaces::g_Engine->GetLocalPlayer();
	if (!local_id) return;

	player_info_t localPlayerInfo;
	if (!Interfaces::g_Engine->GetPlayerInfo(local_id, &localPlayerInfo)) return;

	C_BaseEntity* local = (C_BaseEntity*)Interfaces::g_EntityList->GetClientEntity(local_id);
	if (!local) return;
	...
	auto glove = reinterpret_cast<CGloves*>(Interfaces::g_EntityList->GetClientEntityFromHandle(wearables[0]));

	if (!glove)
	{
		auto our_glove = reinterpret_cast<CGloves*>(Interfaces::g_EntityList->GetClientEntityFromHandle(glove_handle));
	...
	if (!glove)
	{
		int iEnt = Interfaces::g_EntityList->GetHighestEntityIndex() + 1;
		...
		if (pWeapon)
		{
			...
			*(DWORD*)((DWORD)local + NetVarManager->GetOffset("DT_BaseCombatCharacter", "m_hMyWearables")) = m_Index;
			glove = (CGloves*)Interfaces::g_EntityList->GetClientEntity(iEnt);
		}

		{
			static auto set_abs_origin_fn = reinterpret_cast<void(__thiscall*)(void*, const Vector&)>
				(U::FindPattern("client_panorama.dll", (BYTE*)"\x55\x8B\xEC\x83\xE4\xF8\x51\x53\x56\x57\x8B\xF1", "xxxxxxxxxxxx"));
	...
	*reinterpret_cast<int*>(uintptr_t(glove) + 0x64) = -1;
	...
	if (glove)
	{
		...
		case 1: //Bloodhound Gloves
		{
			if (*glove->m_AttributeManager()->m_Item()->ItemDefinitionIndex() != 5027 || *glove->FallbackPaintKit() != GetGloveSkinByMenu(0, g_Options.Skinchanger.gloveskin))
			{
				...
				*glove->AccountID() = localPlayerInfo.xuid;
				glove->SetGloveModelIndex(Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_models/arms/glove_bloodhound/v_glove_bloodhound.mdl"));
		...
		case 2: //Sport Gloves
		{
			...
			{
				glove->SetGloveModelIndex(Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_models/arms/glove_sporty/v_glove_sporty.mdl"));
		...
		case 3://SLICK GLOVES
		{
			...
			{
				glove->SetGloveModelIndex(Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_models/arms/glove_slick/v_glove_slick.mdl"));
		...
		case 4://LEATHER HANDWRAPS
		{
			...
			{
				glove->SetGloveModelIndex(Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_models/arms/glove_handwrap_leathery/v_glove_handwrap_leathery.mdl"));
		...
		case 5://MOTORCYCLE GLOVES
		{
			...
			{
				glove->SetGloveModelIndex(Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_models/arms/glove_motorcycle/v_glove_motorcycle.mdl"));
		...
		case 6://SPECIALIST GLOVES
		{
			...
			{
				glove->SetGloveModelIndex(Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_models/arms/glove_specialist/v_glove_specialist.mdl"));
		...
		case 7://HYDRA GLOVES
		{
			...
			{
				glove->SetGloveModelIndex(Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_models/arms/glove_bloodhound/v_glove_bloodhound_hydra.mdl"));
