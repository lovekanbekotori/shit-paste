...
public:
	...
	{
		C_BaseEntity* g_LocalPlayer = Interfaces::g_EntityList->GetClientEntity(Interfaces::g_Engine->GetLocalPlayer());
		...
		auto studio_model = Interfaces::g_ModelInfo->GetStudiomodel(player->GetModel());
		if (!studio_model)
		{
			return false;
		}
		...
		for (int i = 0; i < ARRAYSIZE(scan_hitboxes); i++)
		{
			...
			if (hitbox)
			{
				...
				Interfaces::g_EngineTrace->TraceRay(ray, MASK_SHOT | CONTENTS_GRATE, &filter, &tr);
...
private:
	...
	void DrawAimdirection(C_BaseEntity* pEntity, Color color);
