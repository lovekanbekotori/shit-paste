...
void legitbot::OnCreateMove(CInput::CUserCmd *pCmd, C_BaseEntity *local, bool& bSendPacket)
{
	...
	{
		if(g_Options.LegitAA.legitaaenable)
			do_legitaa(pCmd, bSendPacket, local);
...
void legitbot::do_legitaa(CInput::CUserCmd * pCmd, bool& bSendPacket, C_BaseEntity* local)
{
	CBaseCombatWeapon* pWeapon = (CBaseCombatWeapon*)Interfaces::g_EntityList->GetClientEntityFromHandle(local->GetActiveWeaponHandle());
	...
	if ((pCmd->buttons & IN_USE) || local->GetMoveType() == MOVETYPE_LADDER || local->GetMoveType() == MOVETYPE_NOCLIP || pCmd->buttons & IN_ATTACK || pCmd->buttons & IN_ATTACK2 || !!MiscFunctions::IsGrenade(pWeapon))
		return;
	...
	if (g_Options.LegitAA.legitaatype > 0)
	{
...
void legitbot::triggerbot(CInput::CUserCmd *cmd, C_BaseEntity* local, CBaseCombatWeapon* weapon)
{
	...
	Interfaces::g_EngineTrace->TraceRay(Ray, MASK_SHOT, &Filter, &Trace);
...
void legitbot::do_aimbot(C_BaseEntity *local, CBaseCombatWeapon *weapon, CInput::CUserCmd *cmd)
{
	...
	C_BaseEntity* entity = (C_BaseEntity*)Interfaces::g_EntityList->GetClientEntity(best_target);
	if (!entity)
		return;
...
bool legitbot::hit_chance(C_BaseEntity* local, CInput::CUserCmd* cmd, CBaseCombatWeapon* weapon, C_BaseEntity* target)
{
	...
	for (int i = 0; i < max_traces; i++) {
		...
		Interfaces::g_EngineTrace->ClipRayToEntity(ray, MASK_SHOT | CONTENTS_GRATE, target, &tr);
...
void legitbot::weapon_settings(CBaseCombatWeapon* weapon)
{
	...
	if (MiscFunctions::IsHeavy(weapon))
	{
		aim_key = g_Options.LegitBot.MainKey;
...
bool get_hitbox_pos(C_BaseEntity* entity, int hitbox, Vector &output)
{
	...
	studiohdr_t *studioHdr = Interfaces::g_ModelInfo->GetStudiomodel(model);
	if (!studioHdr)
		return false;
...
int legitbot::get_target(C_BaseEntity *local, CBaseCombatWeapon *weapon, CInput::CUserCmd *cmd, Vector &destination)
{
	...
	Interfaces::g_Engine->GetViewAngles(view_angle);

	for (int i = 1; i <= Interfaces::g_Globals->maxClients; i++)
	{
		C_BaseEntity *entity = (C_BaseEntity*)Interfaces::g_EntityList->GetClientEntity(i);
		if (!entity
			|| entity == local
			|| entity->IsDormant()
