...
bool HandleBulletPenetration(CSWeaponInfo *wpn_data, FireBulletData &data)
{
	surfacedata_t *enter_surface_data = Interfaces::g_PhysProps->GetSurfaceData(data.enter_trace.surface.surfaceProps);
	...
	surfacedata_t *exit_surface_data = Interfaces::g_PhysProps->GetSurfaceData(trace_exit.surface.surfaceProps);
...
bool CanHit(C_BaseEntity* entity, const Vector &point, float *damage_given)
{
	...
	if (SimulateFireBullet(entity, local, reinterpret_cast<CBaseCombatWeapon*>(Interfaces::g_EntityList->GetClientEntityFromHandle(static_cast<HANDLE>(local->GetActiveWeaponHandle()))), data))
	{
...
bool CanWallbang(int &dmg)
{
	...
	auto *local = Interfaces::g_EntityList->GetClientEntity(Interfaces::g_Engine->GetLocalPlayer());
	CBaseCombatWeapon* weapon = (CBaseCombatWeapon*)Interfaces::g_EntityList->GetClientEntityFromHandle((HANDLE)local->GetActiveWeaponHandle());
	if (!local)
		return false;
	...
	Vector EyeAng;
	Interfaces::g_Engine->GetViewAngles(EyeAng);
