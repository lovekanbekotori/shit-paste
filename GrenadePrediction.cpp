...
void CCSGrenadeHint::Tick(int buttons)
{
	...
	{
		if (!g_Options.Visuals.Grenadetrajectory)
			return;
	...
	if (g_Options.Visuals.Active)
	{
		...
		if ((type) && path.size() > 1)
		{
			...
			for (auto it = path.begin(), end = path.end(); it != end; ++it)
			{
				...
				{
					Interfaces::g_Surface->DrawSetColor(lineColor);
					Interfaces::g_Surface->DrawLine((int)nadeStart.x, (int)nadeStart.y, (int)nadeEnd.x, (int)nadeEnd.y);
				}
				prev = *it;
			}
			...
			if (g_Render->WorldToScreen(prev, nadeEnd))
			{
				...
				Interfaces::g_Surface->DrawSetColor(circleLine);
				Interfaces::g_Surface->DrawOutlinedCircle((int)nadeEnd.x, (int)nadeEnd.y, 10, 48);
			}
		}
	}
}
...
void CCSGrenadeHint::Simulate(CViewSetup* setup)
{
	if (g_Options.Visuals.Active)
	{
		...
		Vector angles; Interfaces::g_Engine->GetViewAngles(angles);
		Setup(vecSrc, vecThrow, angles);

		float interval = Interfaces::g_Globals->interval_per_tick;
...
void CCSGrenadeHint::TraceHull(Vector& src, Vector& end, trace_t& tr)
{
	if (g_Options.Visuals.Active)
	{
		...
		filter.pSkip = Interfaces::g_EntityList->GetClientEntity(Interfaces::g_Engine->GetLocalPlayer());

		Ray_t ray;
		ray.Init(src, end, hull[0], hull[1]);

		Interfaces::g_EngineTrace->TraceRay(ray, 0x200400B, &filter, &tr);
	}
}
