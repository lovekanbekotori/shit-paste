...
void Interfaces::Initialise()
{
	...
	auto ClientPointer = get_module_factory(GetModuleHandleW(L"client_panorama.dll"));
	...
	g_MoveHelper = **(IMoveHelper***)(U::FindPattern("client_panorama.dll", (PBYTE)"\x8B\x0D\x00\x00\x00\x00\x8B\x46\x08\x68", "xx????xxxx") + 2);
	...
	g_Input = *(CInput**)(U::pattern_scan(GetModuleHandleW(L"client_panorama.dll"), "B9 ? ? ? ? F3 0F 11 04 24 FF 50 10") + 1);
	g_ViewRender = *(IViewRender**)(U::FindPattern("client_panorama.dll", (PBYTE)"\xA1\x00\x00\x00\x00\xB9\x00\x00\x00\x00\xC7\x05\x00\x00\x00\x00\x00\x00\x00\x00\xFF\x10", "x????x????xx????????xx") + 1);
	g_PlayerResource = **(C_CSPlayerResource***)(U::FindPattern("client_panorama.dll", (PBYTE)"\x8B\x3D\x00\x00\x00\x00\x85\xFF\x0F\x84\x00\x00\x00\x00\x81\xC7", "xx????xxxx????xx") + 2);
	g_GameRules = **(C_CSGameRules***)(U::FindPattern("client_panorama.dll", (PBYTE)"\xA1\x00\x00\x00\x00\x8B\x0D\x00\x00\x00\x00\x6A\x00\x68\x00\x00\x00\x00\xC6\x80", "x????xx????xxx????xx") + 1);
	...
	g_pViewRenderBeams = *reinterpret_cast< IViewRenderBeams** > (U::FindSig(("client_panorama.dll"), ("A1 ? ? ? ? 56 8B F1 B9 ? ? ? ? FF 50 08")) + 0x1);
	...
	g_GlowObjManager = *(CGlowObjectManager**)(U::pattern_scan(GetModuleHandleW(L"client_panorama.dll"), "0F 11 05 ? ? ? ? 83 C8 01 C7 05 ? ? ? ? 00 00 00 00") + 3);
