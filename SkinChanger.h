...
int GetCurrentKnifeModel(int currentKnife)
{
	...
	{
	case 0:
		return Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_bayonet.mdl"); //Bayo
	case 1:
		return Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_flip.mdl"); //Flip
	case 2:
		return Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_gut.mdl"); //Gut
	case 3:
		return Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_karam.mdl"); //Karambit
	case 4:
		return Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_m9_bay.mdl"); //M9 Bayo
	case 5:
		return Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_tactical.mdl"); //Huntsman
	case 6:
		return Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_falchion_advanced.mdl"); //Falchion
	case 7:
		return Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_survival_bowie.mdl"); //Bowie
	case 8:
		return Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_butterfly.mdl"); //Butterfly
	case 9:
		return Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_push.mdl"); //Daggers
	case 10:
		return Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_gg.mdl"); //GunGame
	default:
		return 0;
	}
}
...
void SkinChanger()
{
	...
	{
		C_BaseEntity *pEntity = Interfaces::g_EntityList->GetClientEntityFromHandle(weapons[i]);
		if (pEntity)
		{
				...
				if (g_Options.Skinchanger.knifemodel > 0)
				{
					if (pEntity->GetClientClass()->m_ClassID)
					{
						if (pEntity->GetClientClass()->m_ClassID == (int)ClassID::CKnife)
						{
							...
							if (worldmodel_handle) worldmodel = (CBaseCombatWeapon*)Interfaces::g_EntityList->GetClientEntityFromHandle(worldmodel_handle);
