...
void Hooked_RecvProxy_Viewmodel(CRecvProxyData *pData, void *pStruct, void *pOut)
{
	...
	int default_t = Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_default_t.mdl");
	int default_ct = Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_default_ct.mdl");
	int iBayonet = Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_bayonet.mdl");
	int iButterfly = Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_butterfly.mdl");
	int iFlip = Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_flip.mdl");
	int iGut = Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_gut.mdl");
	int iKarambit = Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_karam.mdl");
	int iM9Bayonet = Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_m9_bay.mdl");
	int iHuntsman = Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_tactical.mdl");
	int iFalchion = Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_falchion_advanced.mdl");
	int iDagger = Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_push.mdl");
	int iBowie = Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_survival_bowie.mdl");

	int iGunGame = Interfaces::g_ModelInfo->GetModelIndex("models/weapons/v_knife_gg.mdl");
...
void SetViewModelSequence2(const CRecvProxyData *pDataConst, void *pStruct, void *pOut)
{
	...
	if (pViewModel)
	{
		C_BaseEntity* pOwner = static_cast<C_BaseEntity*>(Interfaces::g_EntityList->GetClientEntity(pViewModel->GetOwner() & 0xFFF));

		// Compare the owner entity of this view model to the local player entity.
		if (pOwner && pOwner->GetIndex() == Interfaces::g_Engine->GetLocalPlayer())
		{
			// Get the filename of the current view model.
			const model_t* pModel = Interfaces::g_ModelInfo->GetModel(pViewModel->GetModelIndex());
			std::string szModel = Interfaces::g_ModelInfo->GetModelName(pModel);
...
{
	for (ClientClass* pClass = Interfaces::g_CHLClient->GetAllClasses(); pClass; pClass = pClass->m_pNext) {
