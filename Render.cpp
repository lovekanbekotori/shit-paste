...
void Render::SetupFonts()
{
	...
	font.ESP = Interfaces::g_Surface->FontCreate();
	font.Defuse = Interfaces::g_Surface->FontCreate();
	font.Icon = Interfaces::g_Surface->FontCreate();
	font.Guns = Interfaces::g_Surface->FontCreate();

	Interfaces::g_Surface->SetFontGlyphSet(font.ESP, "Calibri", 15, 100, 0, 0, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW | FONTFLAG_OUTLINE);
	Interfaces::g_Surface->SetFontGlyphSet(font.Defuse, "Tahoma", 16, 100, 0, 0, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW | FONTFLAG_OUTLINE);
	Interfaces::g_Surface->SetFontGlyphSet(font.Icon, "astriumwep", 18, 100, 0, 0, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW | FONTFLAG_OUTLINE);
	Interfaces::g_Surface->SetFontGlyphSet(font.Guns, "Calibri", 13, 100, 0, 0, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW | FONTFLAG_OUTLINE);
}
...
void Render::Clear(int x, int y, int w, int h, Color color)
{
	...
	Interfaces::g_Surface->DrawFilledRect(x, y, x + w, y + h);
}

void Render::DrawOutlinedRect(int x, int y, int w, int h, Color col)
{
	Interfaces::g_Surface->DrawSetColor(col);
	Interfaces::g_Surface->DrawOutlinedRect(x, y, x + w, y + h);
}


void Render::DrawRect(int x1, int y1, int x2, int y2, Color clr)
{
	Interfaces::g_Surface->DrawSetColor(clr);
	Interfaces::g_Surface->DrawFilledRect(x1, y1, x2, y2);
}
...
void Render::DrawString2(DWORD font, int x, int y, Color color, DWORD alignment, const char* msg, ...)
{
	...
	int width, height;
	Interfaces::g_Surface->GetTextSize(font, wbuf, width, height);
	...
	Interfaces::g_Surface->DrawSetTextFont(font);
	Interfaces::g_Surface->DrawSetTextColor(r, g, b, a);
	Interfaces::g_Surface->DrawSetTextPos(x, y - height / 2);
	Interfaces::g_Surface->DrawPrintText(wbuf, wcslen(wbuf));
}
...
{
	Interfaces::g_Surface->DrawSetColor(col);
	Interfaces::g_Surface->DrawFilledRect(x, y, x + 1, y + 1);
}
...
void Render::Line(int x, int y, int x2, int y2, Color color)
{
	...
	Interfaces::g_Surface->DrawSetColor(color);
	Interfaces::g_Surface->DrawLine(x, y, x2, y2);
}

void Render::PolyLine(int *x, int *y, int count, Color color)
{
	Interfaces::g_Surface->DrawSetColor(color);
	Interfaces::g_Surface->DrawPolyLine(x, y, count);
}

bool Render::WorldToScreen(Vector &in, Vector &out)
{
	const matrix3x4& worldToScreen = Interfaces::g_Engine->WorldToScreenMatrix(); //Grab the world to screen matrix from CEngineClient::WorldToScreenMatrix
...
{
	float w;
	const matrix3x4 &worldToScreen = Interfaces::g_Engine->WorldToScreenMatrix();
...
bool Render::WorldToScreen2(const Vector &vOrigin, Vector &vScreen)
{
	...
	Interfaces::g_Engine->GetScreenSize(iScreenWidth, iScreenHeight);
...
void Render::Text(int x, int y, Color color, DWORD font, const char* text, ...)
{
	...
	Interfaces::g_Surface->DrawSetTextColor(color);
	Interfaces::g_Surface->DrawSetTextPos(x, y);
	Interfaces::g_Surface->DrawPrintText(wcstring, wcslen(wcstring));
}
...
void Render::Text(int x, int y, Color color, DWORD font, const wchar_t* text)
{
	...
	Interfaces::g_Surface->DrawSetTextColor(color);
	Interfaces::g_Surface->DrawSetTextPos(x, y);
	Interfaces::g_Surface->DrawPrintText(text, wcslen(text));
}
...
RECT Render::GetTextSize(DWORD font, const char* text)
{
	...
	RECT rect;
	int x, y;
	Interfaces::g_Surface->GetTextSize(font, WideBuffer, x, y);
	rect.left = x;
	rect.bottom = y;
	rect.right = x;
	return rect;
...
{
	static int Texture = Interfaces::g_Surface->CreateNewTextureID(true); //need to make a texture with procedural true
	unsigned char buffer[4] = { 255, 255, 255, 255 };//{ color.r(), color.g(), color.b(), color.a() };

	Interfaces::g_Surface->DrawSetTextureRGBA(Texture, buffer, 1, 1); //Texture, char array of texture, width, height
	Interfaces::g_Surface->DrawSetColor(color); // keep this full color and opacity use the RGBA @top to set values.
	Interfaces::g_Surface->DrawSetTexture(Texture); // bind texture

	Interfaces::g_Surface->DrawTexturedPolygon(count, Vertexs);
}
...
{
	static int texture_id = Interfaces::g_Surface->CreateNewTextureID(true);
	static unsigned char buf[4] = { 255, 255, 255, 255 };
	Interfaces::g_Surface->DrawSetTextureRGBA(texture_id, buf, 1, 1);
	Interfaces::g_Surface->DrawSetColor(col);
	Interfaces::g_Surface->DrawSetTexture(texture_id);
	Interfaces::g_Surface->DrawTexturedPolygon(n, vertice);
}
void Render::OutlineCircle(int x, int y, int r, int seg, Color color)
{
	Interfaces::g_Surface->DrawSetColor(0, 0, 0, 255);
	Interfaces::g_Surface->DrawOutlinedCircle(x, y, r - 1, seg);
	Interfaces::g_Surface->DrawOutlinedCircle(x, y, r + 1, seg);
	Interfaces::g_Surface->DrawSetColor(color);
	Interfaces::g_Surface->DrawOutlinedCircle(x, y, r, seg);
}
...
void Render::TexturedPolygon(int n, std::vector<Vertex_t> vertice, Color color) {
	...
	Interfaces::g_Surface->DrawSetColor(color);
	Interfaces::g_Surface->DrawSetTexture(texture_id);
	Interfaces::g_Surface->DrawTexturedPolygon(n, vertice.data());
}
...
void Render::Draw3DCube(float scalar, QAngle angles, Vector middle_origin, Color outline)
{
	...
	Interfaces::g_Surface->DrawSetColor(outline);

	// Back frame
	Interfaces::g_Surface->DrawLine(points_screen[0].x, points_screen[0].y, points_screen[1].x, points_screen[1].y);
	Interfaces::g_Surface->DrawLine(points_screen[0].x, points_screen[0].y, points_screen[2].x, points_screen[2].y);
	Interfaces::g_Surface->DrawLine(points_screen[3].x, points_screen[3].y, points_screen[1].x, points_screen[1].y);
	Interfaces::g_Surface->DrawLine(points_screen[3].x, points_screen[3].y, points_screen[2].x, points_screen[2].y);

	// Frame connector
	Interfaces::g_Surface->DrawLine(points_screen[0].x, points_screen[0].y, points_screen[4].x, points_screen[4].y);
	Interfaces::g_Surface->DrawLine(points_screen[1].x, points_screen[1].y, points_screen[5].x, points_screen[5].y);
	Interfaces::g_Surface->DrawLine(points_screen[2].x, points_screen[2].y, points_screen[6].x, points_screen[6].y);
	Interfaces::g_Surface->DrawLine(points_screen[3].x, points_screen[3].y, points_screen[7].x, points_screen[7].y);

	// Front frame
	Interfaces::g_Surface->DrawLine(points_screen[4].x, points_screen[4].y, points_screen[5].x, points_screen[5].y);
	Interfaces::g_Surface->DrawLine(points_screen[4].x, points_screen[4].y, points_screen[6].x, points_screen[6].y);
	Interfaces::g_Surface->DrawLine(points_screen[7].x, points_screen[7].y, points_screen[5].x, points_screen[5].y);
	Interfaces::g_Surface->DrawLine(points_screen[7].x, points_screen[7].y, points_screen[6].x, points_screen[6].y);
}
