...
void DrawRadar()
{
		...
		if (ImGui::Begin(("Radar"), &g_Options.Misc.radarwindow, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize))
		{
			...
			if (Interfaces::g_Engine->IsInGame() && Interfaces::g_Engine->IsConnected())
			{
				Vector LocalPos = pLocal->GetEyePosition();
				Vector ang;
				Interfaces::g_Engine->GetViewAngles(ang);

				for (int i = 0; i < Interfaces::g_Engine->GetMaxClients(); i++) {
					C_BaseEntity* pBaseEntity = Interfaces::g_EntityList->GetClientEntity(i);
