...
{
	m_pConVar = Interfaces::g_CVar->FindVar(szCVar);
	m_newvalue = newvalue;
...
MinspecCvar::~MinspecCvar()
{
	if (ValidCvar())
	{
		Interfaces::g_CVar->UnregisterConCommand(m_pConVar);
		m_pConVar->pszName = m_szOriginalName;
		m_pConVar->SetValue(m_OriginalValue);
		Interfaces::g_CVar->RegisterConCommand(m_pConVar);
	}
}
...
SpoofedConvar::SpoofedConvar(const char* szCVar) {
	m_pOriginalCVar = Interfaces::g_CVar->FindVar(szCVar);
	Spoof();
}
...
SpoofedConvar::~SpoofedConvar() {
	if (IsSpoofed()) {
		...
		Interfaces::g_CVar->UnregisterConCommand(m_pDummyCVar);
		free(m_pDummyCVar);
		m_pDummyCVar = NULL;
	}
...
void SpoofedConvar::Spoof() {
	if (!IsSpoofed() && m_pOriginalCVar) {
		...
		//Register it
		Interfaces::g_CVar->RegisterConCommand(m_pDummyCVar);
