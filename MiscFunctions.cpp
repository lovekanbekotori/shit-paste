...
void UTIL_TraceLine(const Vector& vecAbsStart, const Vector& vecAbsEnd, unsigned int mask, C_BaseEntity *ignore, int collisionGroup, trace_t *ptr)
{
	...
	Interfaces::g_EngineTrace->TraceRay(ray, mask, &traceFilter, ptr);
...
void UTIL_ClipTraceToPlayers(C_BaseEntity* pEntity, Vector start, Vector end, unsigned int mask, ITraceFilter* filter, trace_t* tr)
{
	...
	Interfaces::g_EngineTrace->ClipRayToEntity(ray, mask | CONTENTS_HITBOX, pEntity, &playerTrace);
...
bool MiscFunctions::IsSmg(void* weapon)
{
	...
	if (pWeaponClass->m_ClassID == (int)ClassID::CWeaponMP7 || pWeaponClass->m_ClassID == (int)ClassID::CWeaponMP9 || /*pWeaponClass->m_ClassID == (int)ClassID::CWeaponMP5SD ||*/ pWeaponClass->m_ClassID == (int)ClassID::CWeaponUMP45 || pWeaponClass->m_ClassID == (int)ClassID::CWeaponP90 || pWeaponClass->m_ClassID == (int)ClassID::CWeaponBizon || pWeaponClass->m_ClassID == (int)ClassID::CWeaponMAC10)
		return true;
	else
		return false;
}
...
void SayInChat(const char *text)
{
	...
	Interfaces::g_Engine->ClientCmd_Unrestricted(buffer);
}
...
Vector GetHitboxPosition(C_BaseEntity* pEntity, int Hitbox)
{
    ...
    studiohdr_t* hdr = Interfaces::g_ModelInfo->GetStudiomodel(pEntity->GetModel());
